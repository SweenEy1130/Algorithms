...
// class Solution {
// public:
//     vector<vector<string> > findLadders(string end, string start, unordered_set<string> &dict) {
//         int word_len, word_cnt = dict.size(), max_depth = 999999;
//         bool flag = true;
//         vector<vector<string> > ans;
//         map<string, int> word_map;
//         qnode q[100000];
//         qnode tnode(start, -1, 1);
//         q[0] = tnode;
...
//             head++;
//             tnode = q[head];
//             string str_head = tnode.word;
//             word_len = str_head.length();
...
//                     if (ch != c){
//                         str_head[i] = c;
//                         if (str_head == end && max_depth >= tnode.depth){
//                             flag = false;
//                             max_depth = tnode.depth;
//                             int ptr = head;
//                             vector<string> res;
//                             res.push_back(end);
//                             while (ptr != -1){
//                                 res.push_back(q[ptr].word);
//                                 ptr = q[ptr].prev;
//                             }
//                             ans.push_back(res);
//                         }

//                         if (flag && dict.find(str_head) != dict.end()){
//                             string str_map1 = tnode.word + str_head;
//                             string str_map2 = str_head + tnode.word;

//                             if (word_map.find(str_map1) == word_map.end()){
//                                 word_map[str_map1] = tnode.depth;
//                                 word_map[str_map2] = tnode.depth;
//                                 tail++;
//                                 qnode pnode(str_head, head, tnode.depth + 1);
//                                 q[tail] = pnode;
//                             }else
//                             if (tnode.depth == word_map[str_map1]){
//                                 tail++;
//                                 qnode pnode(str_head, head, tnode.depth + 1);
//                                 q[tail] = pnode;
//                             }
    ...
    vector<vector<string> > findLadders(string end, string start, unordered_set<string> &dict) {
        ...
        while (head <= tail){
            ...
            for (auto itr = dict.begin(); itr != dict.end(); ++itr) {
                ...
                for (int i = 0; i < word_len; ++i){
                    if (distance <= 1){
                        if ((*itr)[i] != str_head[i]) distance++;
                    }else break;
                }

                if (distance == 1){
                    if (*itr == end && max_depth >= tnode.depth){
                        flag = false;
                        max_depth = tnode.depth;
                        int ptr = head;
                        vector<string> res;
                    ...
                    if (flag && dict.find(*itr) != dict.end()){
                        string str_map1 = tnode.word + *itr;
                        string str_map2 = *itr + tnode.word;

                        if (word_map.find(str_map1) == word_map.end()){
                            word_map[str_map1] = tnode.depth;
                            word_map[str_map2] = tnode.depth;
                            tail++;
                            qnode pnode(*itr, head, tnode.depth + 1);
                            q[tail] = pnode;
                        }else
                        if (tnode.depth == word_map[str_map1]){
                            tail++;
